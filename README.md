...
| `pi` | `~/.pi/` | Pi 에이전트 — cmux 알림, `.env` 차단, pi-nvim 통합 |
...
Pi 로컬 확장 의존성은 git clone/stow 후 별도 설치가 필요합니다:
...
Lazyweb MCP는 토큰을 로컬에만 저장합니다. 토큰은 git에 커밋하지 않습니다:

```bash
mkdir -p ~/.lazyweb
printf '%s' '<LAZYWEB_TOKEN>' > ~/.lazyweb/lazyweb_mcp_token
chmod 600 ~/.lazyweb/lazyweb_mcp_token
```

Pi 로컬 확장 (`~/.pi/agent/extensions/`):
- **lazyweb-mcp.ts** — Lazyweb MCP 기반 UI 레퍼런스/스크린샷 검색 도구 (`lazyweb_search`, `lazyweb_health` 등)
